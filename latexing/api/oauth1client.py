...
class OAuth1Client():
    ...
    def __init__(self, client_key, client_secret, headers={}):
        self.client_key = client_key
        self.client_secret = client_secret
        self.headers = headers

    def request(self, url, **args):
        client = oauthlib.oauth1.Client(self.client_key, client_secret=self.client_secret, **args)
        uri, headers, body = client.sign(url)
        headers.update(self.headers)
        log.trace("%s %s", uri, headers)
